...
- Verification for every change is exactly:
  `uv run ruff check . && uv run mypy src && uv run mypy --platform linux src && uv run mypy --platform win32 src && uv run python scripts/check_guardrails.py && QT_QPA_PLATFORM=offscreen uv run pytest -q`.
